...
exports.run = async (client, message, args, ops) => {
    console.log(ops);
    if (!message.member.hasPermission("MANAGE_MESSAGES")) return message.channel.send("You Don't Have Permissions To Perform This Command!")
    if (!message.member.voiceChannel) return message.channel.send('Connectes toi à un salon vocal');


    if (!args[0]) return message.channel.send('Please Song Name.');
